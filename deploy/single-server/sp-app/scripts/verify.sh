...
verify_target_service() {
  ...
  [ "${health_status}" = "healthy" ] || [ "${health_status}" = "running" ] || {
    echo "验证失败：${service_name} 状态异常：${health_status}。" >&2
    exit 1
  }
...
for required_text in '"success":true' '"status":"ok"'; do
  case "${response_body}" in *"${required_text}"*) ;; *) echo "验证失败：/health/ready 缺少 ${required_text}。" >&2; exit 1 ;; esac
done
wait_for_http "/health/dependencies" http://127.0.0.1/health/dependencies
case "${response_body}" in *'"success":true'*) ;; *) echo "验证失败：依赖健康接口不是 API JSON。" >&2; exit 1 ;; esac
...
case "${admin_script_text}" in
  *"/api/org/status"*) ;;
  *) echo "验证失败：正式 admin-app.js 缺少组织状态读取逻辑。" >&2; exit 1 ;;
esac
case "${admin_script_text}" in
  *"只读观察"*) ;;
...
case "${workspace_html_body}" in
  *'id="app"'*) ;;
  *) echo "验证失败：组织架构工作区未由当前 Nginx 静态产物提供。" >&2; exit 1 ;;
esac

echo "SP-APP 验证通过：${TARGET_VERSION}。"
