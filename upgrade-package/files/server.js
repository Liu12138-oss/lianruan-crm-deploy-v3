 ...
 * 通过 xxapi.cn 查询企业工商信息
 * 
 * 配置说明：
 *   - 默认使用 xxapi.cn 免费接口，无需 API Key 即可测试（有限额）
 *   - 如需正式使用，在此处配置 COMPANY_API_KEY
 *   - 也可替换为天眼查/企查查/阿里云市场等接口（只需改 buildUrl + parseResult）
 */
const COMPANY_API_KEY = process.env.COMPANY_API_KEY || ''; // 留空则走无 key 模式（部分接口支持）
...
app.get('/api/company-search', async (req, res) => {
  ...
  try {
    ...
    const apiUrl = `https://v2.xxapi.cn/api/companyDetails?keyword=${encodedKeyword}`;
    ...
    if (result.code === 200 && result.data) {
      ...
      const apiResult = {
        ...
        industry: d.industry || '',
        capital: d.capital || '',
        establishDate: d.establishDate ? d.establishDate.split(' ')[0] : '',
    ...
    console.error('[company-search] 外部 API 调用失败:', err.message);
    // 降级：返回本地数据
