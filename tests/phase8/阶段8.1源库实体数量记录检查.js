...
function 主函数() {
  ...
  断言(报告.CRM库.实体统计.明细.every((行) => 行.是否覆盖迁移映射), "测试实体应全部覆盖迁移映射");
  断言(fs.readFileSync(记录文档, "utf8").includes("阶段8.1 V2源库实体数量记录"), "记录文档标题不正确");
  断言(fs.readFileSync(实体CSV路径, "utf8").includes("含敏感键记录数"), "实体CSV缺少敏感键统计字段");
