...
function persistAdminLoginForRedirect(user, token) {
  const adminUser = {
    id: user.id,
    name: user.name,
    role: user.role,
  ...
  localStorage.setItem('admin_api_user', JSON.stringify({ user: adminUser, token }));
}

function redirectAdminUserFromPartnerEntry(user, token) {
  persistAdminLoginForRedirect(user, token);
    ...
    async function doLogin() {
      ...
      try {
        ...
        if (res.user.role === 'admin' || res.user.role === 'superadmin') {
          errMsg.value = '当前账号属于厂商管理入口，正在为您切换。';
          redirectAdminUserFromPartnerEntry(res.user, res.token);
          return;
        }
...
const PartnerSingleSignOnPageV2 = {
  ...
  <div class="login-page">
    ...
    <div class="login-card">
      <div class="login-logo">
        ...
        <h1>渠道单点登录</h1>
        <p>正在校验 IAM 登录凭证 <span class="partner-badge">V2</span></p>
      </div>
  ...
  setup() {
    ...
    function doSingleSignOn() {
      ...
      if (store.user) {
        console.log(`[IAM-SSO][PC][渠道端][${traceId}][流程跳过]`, {
          reason: '当前页面已有渠道登录态',
  ...
  setup() {
    ...
    const quoteCustomerOptions = computed(() => {
      ...
      store.opportunities.filter(isVisibleQuoteOpportunity).forEach(o => {
        ...
        if (existing) {
          existing.opportunityCount = Math.max(existing.opportunityCount || 0, 1);
          return;
        }
        ...
        try {
          ...
          if (result.success && result.data?.length) {
            // 合并数据，保留已有引用
              ...
              normalizeOpportunityList(result.data).forEach(opp => {
