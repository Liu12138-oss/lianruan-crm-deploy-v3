  ...
  <div class="login-page">
    ...
    <div class="login-card">
      ...
      <div class="login-footer">
        <p>员工账号：liujg、wangxh、zhaozq 等 / 123456</p>
        <p style="margin-top:8px;">
          <a href="partner-mobile.html" onclick="sessionStorage.removeItem('force_desktop')" style="color:#999;text-decoration:underline;">切换到手机版 →</a>
        </p>
...
const MainLayout = {
  ...
  setup() {
    ...
    return { store, showNotif, isAdmin, isSuperAdmin, isPartnerAdmin, unreadCount, pendingCount, hotOppCount, reviewCount, pageTitle, go, logout, markAllRead,
                ...
                <table class="quote-table">
                  ...
                  <tbody>
                    <tr v-for="(p,i) in detailItems.filter(p=>p.type==='hardware')" :key="'hw-'+i">
                      ...
                      <td style="font-family:monospace;font-size:12px;color:#888">{{ p.id }}</td>
                      <td>{{ p.unit }}</td>
                      <td v-if="!isStaff">{{ fmt(p.unitPrice) }}</td>
                      <td style="font-weight:700">{{ fmt(p.subtotal) }}</td>
                    </tr>
                  </tbody>
                </table>
  ...
  setup() {
    ...
    function calcUnitPriceForDetail(feat, endpoints) {
      ...
      if (partnerLevel === 'primary' && feat.priceForPrimary) {
        ...
        priceToUse = feat.tiers || feat.priceFixed;
      }
      
      // 固定价格
      if (feat.priceFixed) {
        return typeof priceToUse === 'number' ? priceToUse : feat.priceFixed;
      }
    ...
    const detailItems = computed(() => {
      ...
      productIds.forEach(pid => {
        ...
        if (feat) {
          const unitPrice = calcUnitPriceForDetail(feat, endpoints);
    ...
    function downloadPdf(q) {
      ...
      printHtml += '<style>@page { size: A4; margin: 15mm 12mm; }';
      printHtml += 'body { background: #fff !important; font-family: -apple-system, PingFang SC, Microsoft YaHei, sans-serif; }';
      ...
      printHtml += 'thead { background: #001529; color: #fff; }';
      printHtml += '.quote-header { background: linear-gradient(135deg,#001529,#003a8c); color:#fff; padding: 28px 32px; }';
      printHtml += '.quote-meta { display: flex; flex-wrap: wrap; gap: 14px; margin-top: 16px; }';
      printHtml += '.quote-meta-item { background: rgba(255,255,255,.1); border-radius: 6px; padding: 8px 14px; min-width: 130px; }';
      printHtml += '.quote-meta-item span { font-size: 13px; font-weight: 700; }';
      printHtml += '.quote-body { padding: 20px 32px; }';
      printHtml += '.quote-total { display: flex; justify-content: flex-end; margin-top: 14px; }';
      printHtml += '.total-row.grand { background: #001529; color: #fff; font-weight: 800; font-size: 15px; }';
    ...
    return { kw, detail, adminRegion, isStaff, detailItems, filtered, paginatedData, loading, fmt, view, regenerate, toOrder, downloadPdf, PRODUCT_DATA, getOppName, loadQuotes, allFeaturesForDetail, statusLabelMap,
...
const QuoteNew = {
  ...
  setup() {
    ...
    // 编辑模式：加载已有报价单数据
    if (isEdit) {
      const existingQuote = store.quotes.find(q => q.id === editId);
      if (existingQuote) {
        form.customer = existingQuote.customer || '';
        form.regId = existingQuote.regId || '';
        // 处理商机ID数组：优先使用 oppIds，否则使用 oppId 转换为数组
        if (existingQuote.oppIds && Array.isArray(existingQuote.oppIds)) {
          form.oppIds = [...existingQuote.oppIds];
        } else if (existingQuote.oppId) {
          form.oppIds = [existingQuote.oppId];
        } else {
          form.oppIds = [];
        }
        form.endpoints = existingQuote.endpoints || 100;
        form.validDays = existingQuote.validDays || 30;
        // 加载选中的产品
        if (existingQuote.packageId) selectedPackageId.value = existingQuote.packageId;
        if (existingQuote.products) customFeatures.value = [...existingQuote.products];
        if (existingQuote.hardwareIds) selectedHardware.value = [...existingQuote.hardwareIds];
      }
    }
    ...
    function calculateMaintenancePrice(features, hardwareTotal, endpoints) {
      // 检查是否选择了标准维保服务
      const maintenanceFeature = features.find(f => f.id === 'FEAT-MOD-MAINTENANCE-01-01');
      ...
      // 计算非标准维保功能价格（排除标准维保服务，但包括其他维保模块功能）
      const nonMaintenanceFeatures = features.filter(f => f.id !== 'FEAT-MOD-MAINTENANCE-01-01');
      const featureResult = calcFeatureTotal(nonMaintenanceFeatures, endpoints);
      const nonMaintenanceFeatureTotal = featureResult.total;
      
      // 计算防病毒模块价格
      const antivirusFeature = features.find(f => f.id === 'FEAT-MOD-LEP-03-01');
      let antivirusPrice = 0;
      if (antivirusFeature) {
        // 计算防病毒模块的实际价格
        const qty = endpoints || 0;
        let originalPrice = 0;
        if (antivirusFeature.priceFixed) {
          originalPrice = antivirusFeature.priceFixed;
        } else if (antivirusFeature.tiers && antivirusFeature.tiers.length) {
          const tier = antivirusFeature.tiers.find(t => qty >= t.min && qty <= t.max) || antivirusFeature.tiers[0];
          if (tier) {
        ...
        // 根据渠道商等级选择价格
        let priceToUse = null;
        if (partnerLevel === 'primary' && antivirusFeature.priceForPrimary) {
          priceToUse = antivirusFeature.priceForPrimary;
        } else if (partnerLevel === 'secondary' && antivirusFeature.priceForSecondary) {
          priceToUse = antivirusFeature.priceForSecondary;
        }
        ...
        if (!priceToUse) {
          if (antivirusFeature.priceFixed) {
            actualPrice = antivirusFeature.priceFixed * (antivirusFeature.discount || 1);
          } else if (antivirusFeature.tiers && antivirusFeature.tiers.length) {
            const tier = antivirusFeature.tiers.find(t => qty >= t.min && qty <= t.max) || antivirusFeature.tiers[0];
            if (tier) {
              actualPrice = antivirusFeature.unitPrice ? antivirusFeature.unitPrice * tier.discount : (tier.price || 0);
            }
          }
        } else {
          if (typeof priceToUse === 'number') {
            ...
            if (tier) {
              actualPrice = tier.price || 0;
            }
          }
        }
        
        antivirusPrice = actualPrice * qty;
      }
      
      // 计算其他软硬件价格总和（不包括维保服务）
      const otherTotal = nonMaintenanceFeatureTotal + hardwareTotal;
      
      // 标准维保服务价格 = 其他软硬件价格总和的15% + 防病毒模块价格总和的25%
      const maintenancePrice = (otherTotal * 0.15) + (antivirusPrice * 0.25);
      
      console.log(`[维保价格计算] 其他软硬件总价: ¥${otherTotal}, 防病毒模块价格: ¥${antivirusPrice}, 维保服务价格: ¥${maintenancePrice}`);
      
      return Math.round(maintenancePrice);
    }
    ...
    function calcFeatureTotal(features, endpoints) {
      ...
      features.forEach(feat => {
        ...
        if (partnerLevel === 'primary' && feat.priceForPrimary) {
          // 一级渠道商使用 priceForPrimary
          priceToUse = feat.priceForPrimary;
          priceSource = '一级渠道商价格';
        } else if (partnerLevel === 'secondary' && feat.priceForSecondary) {
          // 二级渠道商使用 priceForSecondary
          priceToUse = feat.priceForSecondary;
          priceSource = '二级渠道商价格';
        }
        ...
        if (!priceToUse) {
          if (feat.priceFixed) {
            // 固定价格
            actualPrice = feat.priceFixed * (feat.discount || 1);
          } else if (feat.tiers && feat.tiers.length) {
            // 阶梯价格
            const tier = feat.tiers.find(t => qty >= t.min && qty <= t.max) || feat.tiers[0];
            if (tier) {
              actualPrice = feat.unitPrice ? feat.unitPrice * tier.discount : (tier.price || 0);
            }
          } else if (feat.unitPrice) {
    ...
    function getFeaturePrice(fid) {
      ...
      // 固定价格
      if (feat.priceFixed) {
        const actualPrice = typeof priceToUse === 'number' ? priceToUse : feat.priceFixed;
        return actualPrice;
      }
    ...
    function calcPriceSummary() {
      ...
      // 计算维保服务价格
      const maintenancePrice = calculateMaintenancePrice(features, hardwareTotal, form.endpoints);
      
      // 计算其他功能价格（排除标准维保服务，但包括其他维保模块功能如原厂现场人工服务）
      const nonMaintenanceFeatures = features.filter(f => f.id !== 'FEAT-MOD-MAINTENANCE-01-01');
      const featureResult = calcFeatureTotal(nonMaintenanceFeatures, form.endpoints);
      
      // 总价 = 其他功能价格 + 硬件价格 + 维保服务价格
      const total = featureResult.total + hardwareTotal + maintenancePrice;
      const originalTotal = featureResult.originalTotal + hardwareTotal + maintenancePrice;
      const discountAmount = originalTotal - total;
      ...
      return {
        originalTotal: Math.round(originalTotal),
        discountAmount: Math.round(discountAmount),
        discountedTotal: Math.round(total)
      };
    }
    ...
    const approvedRegs = computed(() => {
      ...
      // 普通员工：自己创建的 + 被指派给自己的
      if (isStaff.value) {
        return list.filter(r => r.createdBy === userId.value || r.assignedStaffId === userId.value);
      }
      if (adminRegion.value) {
...
const Partners = {
  ...
  setup() {
    ...
    const filtered = computed(() => store.partners.filter(p => {
      const mK = !kw.value || p.name.includes(kw.value) || p.contact.includes(kw.value) || p.phone.includes(kw.value);
...
const OpportunityList = {
  ...
  setup() {
    ...
    const inlineSubTotal = computed(() => {
      ...
      for (const fid of inlineSupplementFeatures.value) total += getFeaturePriceById(fid);
      for (const fid of inlineCustomFeatures.value) total += getFeaturePriceById(fid);
      return total;
    });
