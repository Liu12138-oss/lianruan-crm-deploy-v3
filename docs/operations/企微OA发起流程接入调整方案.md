...
> 文档状态：已完成“渠道产品订单预审”生产 OA 单次人工联调验证，并完成 V3 自动发起、报价单双附件、只读回查和企微自动建群的独立任务实现；任务默认关闭，尚未以 V3 任务进程调用真实外部系统。区管泛微 OA 身份维护能力已交付，但尚未录入任何映射数据；其他业务模板仍须逐一完成字段校验后才能启用。
> 制定日期：2026-08-25。
...
- 采购内容固定为“详见采购订单上传处的报价单”；由服务端生成该订单关联报价的 PDF，并独立上传至两个必填附件字段。
...
组织架构在本项中的最小产物是：可用账号、当前操作者、组织范围校验，以及 V3 用户与企微发起人身份的明确映射。候选核验、人工确认、停用与审计的维护能力已在组织架构账号抽屉交付；候选与正式映射分表保存，本轮没有录入任何记录。企微组织同步尚未完全开放不阻塞本项；但候选、未映射、已停用或映射冲突的用户不得发起流程。
...
1. 当前实际表单中“报价单／收益表上传处”仍显示红色必填星号。首期已改为上传同一份报价单 PDF 至两个附件字段；后续如需不传第二附件，对方须使调整在目标模板生效，并以实际页面或字段元数据和一次不传该字段的测试发起共同确认。
...
| 采购内容             | `4742314689772812367` | 固定字符串“详见采购订单上传处的报价单”。                                                 |
