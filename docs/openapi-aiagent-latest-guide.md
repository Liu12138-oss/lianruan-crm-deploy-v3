...
```text
id, name, partnerLevel, cooperationLevel, level, parentPartnerId, parentPartnerIds,
region, city, bigRegion, status, contact, phone, email,
...
```text
partnerId, partnerName, displayName, partnerLevelName, cooperationLevelName, partnerType, partnerTypeName,
techServiceType, technicalServiceProviderType, isTechnicalServiceProvider
```

字段口径：

- `partnerLevel` 表示渠道分销层级：`primary` 一级、`secondary` 二级、`none` 未设置。
- `cooperationLevel` 表示合作级别：`lep`、`gold`、`silver`、`diamond`；兼容旧字段 `level`。
- `city` 表示渠道商所在城市，渠道商新增/编辑/导入时必填，且必须是地市级行政区名称；用于覆盖率分析，不替代 `region` 和 `bigRegion`。
- 技术服务商/提名技术服务商分析可读取 `techServiceType` 或 `technicalServiceProviderType`。
...
`dimensions` 内统一返回 `byStatus`、`statusDistribution`、`byRegion`、`byBigRegion`、`byPartnerLevel`、`byCooperationLevel`、`byTechServiceType`、`byTechnicalServiceProviderType`、`byPartner`、`byPartnerName`。  
`timeSeries` 按 `dateField + granularity` 分桶，每个周期包含 `count`、`amount`、`byStatus`、`byCooperationLevel`、`byTechServiceType`、`byRegion`、`byBigRegion`。
...
同时返回各资源的 `dimensions` 和 `timeSeries`，可直接用于合作级别、技术服务商类型、状态分布和时间趋势分析。
...
3. 报价数量和金额。
4. 订单数量和金额。
5. 渠道分销层级、合作级别、技术服务商类型和状态字段。

统计分析增强端点均保持只读：`/analytics/{resource}/summary`、`/analytics/business-overview`、`/analytics/partners/profile`、`/analytics/partners/contribution`、`/analytics/regions/contribution`、`/analytics/owners/contribution`、`/analytics/funnel`、`/analytics/funnel/registration-opportunity-order`。

技术服务商也提供独立别名端点，底层来源仍是渠道商数据，默认排除 `techServiceType=none`：
