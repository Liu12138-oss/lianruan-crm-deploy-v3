...
V3 是在 V2 业务体验基础上做工程化、数据库化、部署化改造的 CRM 系统。当前仓库真实形态是“V3 工程化工作区 + V2 兼容层 + 旧版 V2 代码和资产保留”的混合状态。
...
1. 根目录 `README.md` 仍是 V2.2.0 部署说明，不能作为 V3 接管依据。
...
3. `backend/`、`frontend/`、`apps/web/public/`、旧升级包、旧备份脚本中有大量 V2 遗留内容。它们有迁移参考价值，但不应继续作为 V3 主开发入口。
...
`README.md` 标题为“联软渠道管理平台 v2.2.0”，其中仍写：
...
这与 V3 当前 Node 22、PostgreSQL、Redis、Docker Compose 部署形态不一致，必须更新或标记为历史文档。
...
接管建议：只读保留作为迁移对照，不在 V3 新需求中继续修改。
