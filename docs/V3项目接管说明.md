...
| V2 实体映射 | `database/mapping/*.yaml` | 已保留，用于 V3 迁移兼容 |
...
| 测试记录 | `output/playwright/` | 运行时输出，不纳入代码基线 |
...
接管建议：旧后端源码已从主线清理；如需核对迁移规则，以 `database/mapping`、`docs/migration`、`scripts/migration` 和 V3 兼容测试为准。
