...
min_version="${MIN_VERSION:-3.0.0-stage9.26.20260803-kb004}"
...
check_duplicate_phone() {
  ...
  duplicate_count="$(docker compose exec -T postgres psql -U lianruan_app -d lianruan_crm_v3 -tA -c "
    SELECT COUNT(*)
    FROM (
  ...
  if [ -n "${duplicate_count}" ] && [ "${duplicate_count}" != "0" ]; then
    fail "当前数据库已有重复联系电话，请先合并或修正重复账号后再升级。"
  fi
}
...
[ -f "${migration_file}" ] || fail "缺少数据库迁移文件。"
